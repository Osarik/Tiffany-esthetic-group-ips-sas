...
export default function SplashScreen({ onFinish }: { onFinish: () => void }) {
  ...
  useEffect(() => {
    ...
    const t5 = setTimeout(finish, 6800);
    return () => {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-[#FBFBF9] overflow-hidden"
          ...
          <AnimatePresence>
            {phase === 1 && (
              <motion.p
                ...
                className="font-script text-[#2FA79C] text-3xl"
              >
                Bienvenidos
              </motion.p>
            )}
        ...
        <div className="flex items-center justify-center">
          {IMAGES.map((url, i) => (
            <motion.div
              ...
              animate={{ x: POSITIONS[i].x, y: POSITIONS[i].y, scale: 1, opacity: 1 }}
              transition={{
                duration: 1.2,
                delay: i * 0.08,
                ease: [0.16, 1, 0.3, 1],
              }}
              className="absolute w-[90px] h-[90px] rounded-full overflow-hidden border-[3px] border-white shadow-xl"
            >
              <div
